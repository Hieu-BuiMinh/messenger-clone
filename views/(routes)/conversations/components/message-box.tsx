...
import ImageModal from '@/views/(routes)/conversations/components/image-modal'
...
function MessageBox({ isLast, message }: MessageBoxProps) {
	const session = useSession()
	const [imageModalOpen, setImageModalOpen] = useState(false)

	const isOwn = session?.data?.user?.email === message?.sender?.email
	const seenList = (message?.seenUsers || [])
